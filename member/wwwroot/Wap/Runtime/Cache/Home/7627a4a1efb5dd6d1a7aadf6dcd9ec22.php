...
<title><?php echo (L("UserMobile")); ?></title>
...
<div class="con_index">
    ...
    <div class="btn0 <?php echo ($my10); ?>"><a href="/index.php/User/out/"><?php echo (L("Exit")); ?></a></div>
            ...
            <div class="tb1">
            	<div class="title"><?php echo (L("UserMobile")); ?></div>
                
                <div class="c1">
                	<div class="txu1"><?php echo (L("LicensedUser")); ?></div>
                	<div class="txu2"><?php echo (L("Equipment")); ?></div>
					<div class="txu1"><?php echo (L("OneDelete")); ?></div>
					<div class="txu2"><?php echo (L("AllDelete")); ?></div>
                </div>
                
                <?php if(is_array($UserMobile)): $k = 0; $__LIST__ = $UserMobile;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><div class="c<?php echo ($k%2+2); ?>">
                    <div class="txtuser"><?php echo ($vo["wName"]); ?></div>
					<div class="txtuserl"><?php echo ($vo["McName"]); ?></div>
                    <div class="txtuser">
                        <a class="btn2" href="javascript:Jq_del('<?php echo ($vo["Pid"]); ?>')"><?php echo (L("Delete")); ?></a>
                    </div>
					<div class="txtuser">
                        <a class="btn2" href="javascript:Jq_del2('<?php echo ($vo["wUseID"]); ?>')"><?php echo (L("Delete")); ?></a>
                    </div>
...
<script type="text/javascript">
	function Jq_del(Pid){
		$('.jumppage > .tx1').html('<?php echo (L("Confirm_delete")); ?>');
		$('.jumppage_bg, .jumppage').show();
		$('#Jq_sure').click(function(){
			var url = "/index.php/Home/Mobile/delusermobile/Pid/"+Pid;
			window.location.href = url;			
			$(this).attr("disabled","disabled");
		});
	};
	function Jq_del2(wUseID){
		$('.jumppage > .tx1').html('<?php echo (L("Confirm_delete")); ?>');
		$('.jumppage_bg, .jumppage').show();
		$('#Jq_sure').click(function(){
			var url = "/index.php/Home/Mobile/delall/wUseID/"+wUseID;
			window.location.href = url;			
			$(this).attr("disabled","disabled");
		});
	};
